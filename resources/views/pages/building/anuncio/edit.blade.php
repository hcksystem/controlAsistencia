                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="solicitado_por" class="form-control" id="validationCustom01"
                                       value="{{ $anuncio->solicitado_por ?? null}}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="validationCustom02">Correo Solicitante</label>
                                <input type="text" class="form-control" name="correo_contacto" id="validationCustom02" placeholder="" value="{{ $anuncio->correo_contacto ?? null }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="id_servicio">Servicio</label>
                                {!! Form::select('id_servicio',$conceptos, $anuncio->id_servicio ?? null, ['class'=>'custom-select form-control  select2 ', 'id'=>'id_servicio']) !!}
                                 {!! Form::hidden('id_edificio',$building->id, ['class'=>'form-control', 'id'=>'id_edificio']) !!}
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="validationCustom04">Fecha</label>
                               
                                <input type="text" class="form-control" name="fecha_anuncio" id="fecha_anuncio" value="{{ htmlspecialchars_decode(date('d/m/Y', strtotime($anuncio->fecha_anuncio))) }}" required> 
                                  
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="sku">Estatus</label>
                               {!! Form::select('id_status',$estatus, $anuncio->id_status ?? null, ['class'=>'custom-select form-control  select2 ', 'id'=>'estatus']) !!}
                            </div>
                            <div class="col-md-12 m-0">
                                <label for="sku">Título *</label>
                               {!! Form::text('titulo',$anuncio->titulo ?? null, ['class'=>'form-control', 'id'=>'titulo','required']) !!}
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="productDetails">Descripción</label>
                           <textarea class="form-control" id="descripcion"  name="descripcion" rows="8">{{ $anuncio->descripcion ?? null }}</textarea>
                ...
                <div class="row text-right">
                        <div class="col-md-12 ">
                            ...
                            @if(!(Auth::user()->hasRole('corredor')))
                                <button  id="editar" type="submit" class="btn btn-primary"><i><i class="icon-save mr-2"></i>{{__('Guardar Datos')}}</button>
                            @endif
                        </div>
                    </div>
            </form>
